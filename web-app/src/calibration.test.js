...
test('calibration report recursively excludes sensitive and binary source data', () => {
  ...
  const report = createCalibrationReport({
    ...
    analyses: [{ file, latitude: 1.3, longitude: 103.8, captureDate: new Date(), pitch: -20, markerOverride: 'marker', altitudeSource: 'relative', rawMetadata: 'secret', objectUrl: 'blob:secret' }],
  ...
  assert.deepEqual(report.markerOverrides, [{ index: 0, fileName: 'safe.jpg', mode: 'marker' }]);
});
